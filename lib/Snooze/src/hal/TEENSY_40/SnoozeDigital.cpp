...
// Local patch: Teensyduino's imxrt.h gave the IMXRT_GPIOn struct named fields
// (DR, GDIR, IMR, ...). Macros with those exact names now expand inside
// (IMXRT_GPIO6.DR), causing parse errors. Prefix with SNZ_ to avoid collision.
#define SNZ_DR    0
#define SNZ_GDIR  1
#define SNZ_PSR   2
#define SNZ_ICR1  3
    ...
    while ( __builtin_popcountll( _pin ) ) {
        ...
        if ( mode == 3 ) {
            ...
            switch( ( uint32_t )gpio ) {
                case 0x42000000U /* &GPIO6_DR — literal: not constexpr in newer cores */: {
...
void SnoozeDigital::clearIsrFlags( uint32_t ipsr ) {
    ...
    while ( __builtin_popcountll( _pin ) ) {
        ...
        uint32_t status = gpio[SNZ_ISR] & gpio[SNZ_IMR];
        if ( status ) {
            gpio[SNZ_ISR] = status;
        }
...
void SnoozeDigital::attachDigitalInterrupt( uint8_t pin, int type ) {
    ...
    gpio[SNZ_IMR] &= ~mask;    // disable interrupt
    *mux = 5;               // pin is GPIO
    gpio[SNZ_GDIR] &= ~mask;    // pin to input mode
    ...
    if ( type == CHANGE ) {
        ...
        if ( index < 15 ) {
            ...
            gpio[SNZ_ICR1] = (gpio[SNZ_ICR1] & ~( 3 << shift ) ) | ( icr << shift );
        } else {
            uint32_t shift = ( index - 16 ) * 2;
            gpio[SNZ_ICR2] = ( gpio[SNZ_ICR2] & ~( 3 << shift ) ) | ( icr << shift );
        }
    }
    gpio[SNZ_ISR] = mask;  // clear any prior pending interrupt
    gpio[SNZ_IMR] |= mask; // enable interrupt
